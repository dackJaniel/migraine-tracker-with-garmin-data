  ...
  try {
    ...
    const backupData: BackupData = {
      ...
      episodes: episodes.map((ep) => ({
        ...ep,
        startTime: ep.startTime.toISOString(),
        endTime: ep.endTime?.toISOString(),
...
export async function importData(
  ...
  try {
    ...
    // 3. Daten-Konvertierung (ISO Strings → Date Objects)
    const episodes = backupData.episodes.map((ep) => ({
      ...ep,
    ...
    if (mode === 'replace') {
      ...
      await db.transaction('rw', [db.episodes, db.garminData, db.settings], async () => {
        ...
        const existingEpisodeIds = new Set(
          (await db.episodes.toArray()).map((ep) => ep.id).filter((id): id is number => id !== undefined)
        );
        const newEpisodes = episodes.filter((ep) => !ep.id || !existingEpisodeIds.has(ep.id));
 ...
 */
export async function validateBackup(fileContent: string, password: string): Promise<BackupData> {
  const encryptedBackup: EncryptedBackup = JSON.parse(fileContent);
...
async function encryptBackup(
  ...
  const key = await crypto.subtle.importKey('raw', passwordHash, { name: 'AES-GCM' }, false, [
    'encrypt',
  ]);

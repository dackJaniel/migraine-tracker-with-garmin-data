            ...
            <Controller
              ...
              render={({ field }) => (
                <div className="flex items-center justify-between pt-3">
                  <Label htmlFor={symptomKey} className="cursor-pointer">
...
export default function SymptomSelector({
  ...
  const countActiveInCategory = (
    categorySymptoms: readonly string[]
  ): number => {
    return categorySymptoms.filter(
      key => symptoms && symptoms[key as keyof SymptomsFormData]
    ).length;
  };
      ...
      <CardContent className="space-y-3">
        ...
        <div className="border rounded-lg p-3 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {symptoms?.custom?.length > 0 && (
              <Badge variant="outline">{symptoms.custom.length}</Badge>
            )}
          </div>
